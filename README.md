...
> [!IMPORTANT]
> **已安装版本升级提示：**正在使用 `v1.0.1` 或更早版本的设备，请先将应用升级到 `v1.0.2` 或更高版本，再进入「部署」页重新执行一次「一键部署 / 升级」命令。该操作会把应用实际数据目录写入 cron，并安装应用专用的 ping helper，修复非 `/vol1` 卷部署状态和 BusyBox/raw-socket `ping` rc=2 问题。

## 功能

- 工作阶段 + 15 项空闲检查项（CPU、负载、在线用户、SSH 会话、磁盘 IO、网络收发、最小运行时长、SMB 会话、TCP 会话、下载连接、虚拟机、进程、磁盘擦洗、主机在线、日历规则），逐项开关/阈值可配
- 部署向导：执行器一键部署命令、五态健康检测（正常/未部署/版本过旧/运行异常/部署不完整）
