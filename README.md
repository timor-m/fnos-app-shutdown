...
> [!IMPORTANT]
> **已安装版本升级提示：**正在使用低于 `v1.0.1` 版本（即 `v1.0.0` 或更早版本）的设备，请先将应用升级到 `v1.0.1`，再进入「部署」页重新执行一次「一键部署 / 升级」命令。该操作会把应用实际数据目录写入 cron，并通过 `sudo setcap cap_net_raw+ep "$(command -v ping)"` 修复低权限用户执行 `ping` 返回 rc=2 的问题。

## 功能

- 工作阶段 + 15 项空闲检查项（CPU、负载、在线用户、SSH 会话、磁盘 IO、网络收发、最小运行时长、SMB 会话、TCP 会话、下载连接、虚拟机、进程、磁盘擦洗、主机在线、日历规则），逐项开关/阈值可配
- 部署向导：执行器一键部署命令、四态健康检测（正常/未部署/版本过旧/运行异常）
...
- `servicePort`：可选本机服务端口默认值（默认 8366；安装/设置向导可改，0 = 关闭本机直连、仅统一网关）。端口仅绑定 `127.0.0.1`，用于本机部署脚本，不对局域网开放；修改后需重启应用生效

版本号以 `package.json.version` 为准，打包时同步到 `manifest`，并强制与执行器脚本 `SCRIPT_VERSION` 一致（契约 §3.3 部署状态检测依赖这一致性）。
...
安装后在「部署」页复制一键命令，SSH 到设备执行即可完成执行器首次部署（幂等）。命令会自动写入应用实际数据目录，支持安装在任意存储卷。
