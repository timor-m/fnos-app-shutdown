...
<template>
  ...
  <template v-else-if="status">
    ...
    <section class="panel">
      ...
      <p
        v-if="status.executor.state === 'undeployed' || status.executor.state === 'outdated'"
        class="badge-hint warn"
      >
        从应用 v1.0.0 或更早版本升级时，请务必重新执行下方「一键部署 / 升级」命令，将当前应用的实际数据目录写入 cron；仅等待执行器自动更新不能修复旧 cron 配置。
      </p>
    </section>
