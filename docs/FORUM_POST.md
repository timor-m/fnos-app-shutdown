...
- **执行器日志里**（cron 自动运行）：执行器以 root 运行，与 ping 权限无关。rc=2 多为目标地址无路由或解析失败（例如 NAS 不在 `192.168.1.0/24` 网段却填了 `192.168.1.254`）。新版本的提示会附带 ping 自己的报错原文（如 `connect: Network is unreachable`），请核对「主机在线」列表中的地址是否与 NAS 同网段、路由是否可达。
- **手动检测（dry-run）里**：dry-run 以应用用户身份运行，才是权限问题，按下文重新执行部署命令即可。
